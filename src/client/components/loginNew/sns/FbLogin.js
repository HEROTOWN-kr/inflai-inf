...
import { fbGetCredentials } from "../../../lib/fb";
...
function FbLogin() {
  ...
  async function facebookLogin() {
    try {
      const { accessToken, userID } = await fbGetCredentials();

      const res = await axios.post("/api/TB_INFLUENCER/facebookLoginNew", {
        facebookToken: accessToken,
        facebookUserId: userID,
      });

      if (res.status === 200) {
        const { social_type, userToken, userName, userPhoto } = res.data;
