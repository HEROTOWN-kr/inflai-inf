...
const hourData = {
  ...
  datasets: [
    {
      label: "포스트수",
      backgroundColor: ["#EAEAEA", "#18DBA8", "#EAEAEA", "#EAEAEA", "#EAEAEA", "#EAEAEA"],
                ...
                <Grid container justify="space-between">
                  <Grid item xs={12} md={6}>
                    <Typography variant="body1" style={{ textAlign: "center" }}>
                      {"좋아요수: "}
                      <span style={{ color: "blue" }}>{post.like_count}개</span>
                    </Typography>
                  </Grid>
                  <Grid item xs={12} md={6}>
                    <Typography variant="body1" style={{ textAlign: "center" }}>
                      {"댓끌수: "}
                      <span style={{ color: "blue" }}>{post.comments_count}개</span>
                    </Typography>
                  </Grid>
                </Grid>
              </Box>
...
function PostPart(props) {
  ...
  const { mediaData, postStats, lastPosts, maxLikesMedia, maxCmntMedia, INS_ID } = instaData;
  const { hourStats, dayStats, dayMaxIdx, hourMaxIdx, dayAvg, weekAvg } = postStats;
  const classes = useStyles();
  ...
  if (mediaData.urls.length < 6) settings.slidesToShow = mediaData.urls.length;
  ...
  function getGoogleVisionData() {
    ...
    axios
      .get("/api/TB_INSTA/getGoogleDataCommon", {
        params: { INS_ID, host },
      })
      .then((res) => {
  ...
  function getChartData() {
    setProcess(true);
    axios
      .get("/api/TB_INSTA/getGoogleDataApex", {
        params: { INS_ID },
      })
      .then((res) => {
        const { labelInfo, objectInfo } = res.data;
        setApexLabelData({
          ...
          scores: labelInfo.scores,
          colors: labelInfo.detectColors,
        });
        setApexObjectData({
          labels: objectInfo.labels,
          scores: objectInfo.scores,
          colors: objectInfo.detectColors,
        });
        setImgDetectMac({
          description: labelInfo.categoryMax.description,
          value: labelInfo.categoryMax.value,
        });
        setProcess(false);
      })
      .catch((e) => {
          ...
          <Grid item xs={12} md={6}>
            ...
            >
              {process ? (
                ...
                <PieChartApex
                  series={apexLabelData.scores}
                  colors={apexLabelData.colors}
                  labels={apexLabelData.labels}
                />
              )}
            </Box>
          </Grid>
          <Grid item xs={12} md={6}>
            ...
            >
              ...
              {process ? (
                ...
                <PieChartApex
                  series={apexObjectData.scores}
                  colors={apexObjectData.colors}
                  labels={apexObjectData.labels}
                />
              )}
            </Box>
          </Grid>
